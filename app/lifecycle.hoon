...
--
%-  agent:dbug
=|  state-1                ::  bunt state-0 type, which will produce [%0 val=0]  | NEW STATE 1 UPGRADED  |  ANOTHER ONE
...
++  on-init
  ^-  (quip card _this)
  `this(val [69 420])            ::  `this`(our agent core) with value replaced  by 42.  |  NOW ITS [27 32]  | NOPE
::
++  on-save
...
++  on-load
  ...
  ?-  -.old
    %1  `this(state old)                               ::  tests its head for version
    %0  `this(state 1+[val.old val.old])                  ::  NEW STATE
    :: %0  `this(state 2+[val.old val.old])  ::  load it back into the state of our agent if it matches  |  NOW OLD STATE
